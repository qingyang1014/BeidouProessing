...
svnCount = sum([trackResults.status] ~= '-');  %in matlab need []
...
% findSubframeStart(trackResults.status, ...
% trackResults.I_P((settings.skipNumberOfFirstBits+1),:), set_numberOfChnls);
...
for channelNr = activeChnList
    ...
    if (subFrameStart(channelNr) + 30000 -1) > length(trackResults(channelNr).I_P)
        ...
        [eph(trackResults(channelNr).PRN), SOW(trackResults(channelNr).PRN)] = ephemeris(navBitsSamples);
...
%Beidou's SOW, for gps is TOW
transmitTime = SOW(trackResults(activeChnList(1)).PRN);
...
for currMeasNr = 1:fix((settings.msToProcess - max(subFrameStart) -...
    ...
    %                          readyChnList);
    %for debugging at 2014-04-17 comment this line
    
    % Save list of satellites used for position calculation
    navSolutions.channel.SVN(activeChnList, currMeasNr) = trackResults(activeChnList).PRN;
    ...
    subFrameStart + settings.navSolPeriod * (currMeasNr-1),activeChnList, settings);
    % Find satellites positions and clocks corrections =======================
    %edited by LXY at 2014-04-15 atand-->atan2
    ...
    if length(activeChnList) > 3
        ...
        %edited by LXY ,cart2geo,atan --> atan2

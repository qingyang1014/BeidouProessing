...
if (fid > 0)
    ...
    if ((settings.skipAcquisition == 0) || ~exists('acqResults'))
        ...
        data  = fread(fid, dataAdaptCoeff*25*samplesPerCode, settings.dataType);
        ...
        if (dataAdaptCoeff==2)
            ...
            if (settings.switchIQ == 1)
                data=data2 + 1i.*data1;   
                %li is the imag part in matlab,in scilab is %i
            else
                data=data1 + 1i.*data2;
            end
        end
        ...
        plotAcquisition(acqResults);  %for debugging
    end
    
    % Initialize channels and prepare for the run ===========================
    % Start further processing only if a GNSS signal was acquired (the
    % field FREQUENCY will be set to 0 for all not acquired signals)
    if (any(acqResults.carrFreq))     %for in scilab is or
        channel = preRun(acqResults, settings);
        showChannelStatus(channel, settings);
    else
        % No satellites to track, exit
        fprintf('No GNSS signals detected, signal processing finished.\n');
        trackResults = [];
        return;
    end
    ...
    fprintf('   Tracking started at %d:%d:%d\n', H1, MI1, S1);
    
    % Process all channels for given data block
    [trackResults, channel] = tracking(fid, channel, settings);
    
    % Close the data file
    fclose(fid);  %for debugging comment this line
    
    deltaTime = now() - startTime;
    [Y2,M2,D2,H2,MI2,S2]=datevec(deltaTime);
    fprintf('   Tracking is over (elapsed time %d:%d:%d)\n', H2, MI2, S2);
    ...
    save('navSolution.mat','navSolutions','eph');
    fprintf('   Processing is complete for this data block\n');
    ...
    disp('Navigation is over ,not plot')
    %plotNavigation(navSolutions, settings);
    
    fprintf('Post processing of the signal is over.\n');
